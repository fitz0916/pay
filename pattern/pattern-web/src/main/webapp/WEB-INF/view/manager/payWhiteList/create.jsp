...
<div id="createPayWhiteListDialog" class="crudDialog">
	<form id="createPayWhiteListForm" method="post">
		<div class="row">
			<div class="col-sm-12">
		...
		<div class="form-group text-right dialog-buttons">
			...
			<a class="waves-effect waves-button" href="javascript:;" onclick="createPayWhiteListDialog.close();">取消</a>
		</div>
	</form>
</div>
<script>
    function createPayWhiteListSubmit() {
        $.ajax({
            type: 'post',
            url: '${basePath}/payWhiteList/create/${mainAgentId}',
            data: $('#createPayWhiteListForm').serialize(),
            beforeSend: function() {
    			if ($('#address').val() == '') {
    				$('#address').focus();
    				return false;
            ...
            success: function(result) {
            	if (result.model.code == 1) {
                    layer.msg(result.model.data,{icon:1,time:1000},function () {
                    	createPayWhiteListDialog.close();
