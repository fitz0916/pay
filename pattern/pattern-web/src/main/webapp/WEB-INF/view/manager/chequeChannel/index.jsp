...
<div id="main">
	<div id="toolbar">
		...
		<shiro:hasPermission name="pattern:chequeChannel:create">
			<a class="waves-effect waves-button" href="javascript:;" onclick="createAction()"><i class="zmdi zmdi-plus"></i> 新增银行编码</a>
		</shiro:hasPermission>
		
		&nbsp;&nbsp;&nbsp;&nbsp;付款通道：<select id ="queryPayChannel" class="selectpicker" style="width: 150px">
			<option value="0">请选择</option>
		</select>
		&nbsp;&nbsp;&nbsp;&nbsp;银行：<select id ="queryBank" class="selectpicker" style="width: 250px">
			<option value="0">请选择</option>
		</select>
		&nbsp;&nbsp;&nbsp;&nbsp;
...
$(function() {
	...
	$table.bootstrapTable({
		url: '${basePath}/chequeChannel/list',
		height: getHeight(),
		striped: true,
		search: false,
		showRefresh: true,
		...
		columns: [
			...
			{field: 'payChannelId',align:'center', visible:false},
			{field: 'channelName',align:'center', title: '收款通道名称'},
			{field: 'bankLogo',align:'center', title: '银行名称', formatter: function (value, row, index) { return '<img src='+ value +' style="width:126px;height:36px;"/>' }},
			{field: 'bankType',align:'center', title: '所属分类', formatter: function (value, row, index) { return (value=='1')?'借记卡':'信用卡'; }},
			{field: 'bankNum',align:'center', title: '第三方银行编号'},
...
function actionFormatter(value, row, index) {
    return [
		'<shiro:hasPermission name="pattern:chequeChannel:update"><button type="button" class="btn btn-info btn-sm" style="margin-right:10px;padding:0 10px;" onclick="updateRow('+row.id+')">修改</button></shiro:hasPermission>',
		'<shiro:hasPermission name="pattern:chequeChannel:delete"><button type="button" class="btn btn-danger btn-sm" style="padding:0 10px;" onclick="deleteRow('+row.id+',null)">删除</button></shiro:hasPermission>'
    ].join('');
}
// 新增
var createDialog;
...
function createAction() {
	createDialog = $.dialog({
		...
		title: '新增银行编码',
		content: 'url:${basePath}/chequeChannel/create',
...
function updateRow(id){
    updateDialog = $.dialog({
        animationSpeed: 300,
        title: '编辑银行编码',
        content: 'url:${basePath}/chequeChannel/update/' + id,
        onContentReady: function () {
			initMaterialInput();
        }
    });
        ...
        buttons: {
            confirm: {
                ...
                action: function () {
                    ...
                    $.ajax({
                        type: 'get',
                        url: '${basePath}/chequeChannel/delete/' + ids.join("-"),
                        success: function(result) {
